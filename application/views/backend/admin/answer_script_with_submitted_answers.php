    ...
    <element class="col-sm-6 col-aligncenter">
        <div class="pipeline white lined-danger">            
          ...
          ?>
            <ul>
              <?php for ($i = 0; $i < $number_of_options; $i++): ?>
                <li><img width="40px" onclick="showAjaxModal('<?php echo base_url();?>modal/popup/modal_question_image/<?php echo $options[$i];?>');" class="top-10" src="<?php echo base_url();?>public/uploads/online_exam/<?php echo rtrim(trim($options[$i]), ',');?>"></li>
              <?php endfor; ?>
            </ul>
            <?php
              if ($row['submitted_answer'] != "" || $row['submitted_answer'] != null) 
