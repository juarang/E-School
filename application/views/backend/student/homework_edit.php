	                    ...
	                    <div class="col-sm-8">
		                    <div class="pipeline white lined-primary shadow">
    			                ...
    			                <div class="b-t padded-v-big">
    				                <?php echo getEduAppGTLang('file');?>: <a class="text-white btn btn-rounded btn-sm btn-primary" href="<?php echo base_url() . 'public/uploads/homework/' . $row['file_name']; ?>"><i class="os-icon picons-thin-icon-thin-0042_attachment"></i> <?php echo $row['file_name'];?></a>
    			                </div>
			                <?php endif;?>
			                <?php if($row['type'] != 1):?>
    			                <div class="b-t padded-v-big">
    			                    <?php echo form_open(base_url() . 'student/delivery/update_file/'.$delivery_id.'/'.$homework_code , array('enctype' => 'multipart/form-data'));?>
    			                        ...
    			                        <div class="row">
    			                            <div class="col-sm-12">
                                                <textarea class="form-control full-width" placeholder="<?php echo getEduAppGTLang('send_teacher_comment');?>" name="comment" rows="5"><?php echo $query->row()->student_comment;?></textarea>
			                ...
			                <?php if($row['type'] == 1):?>
			                    ...
			                    <?php echo form_open(base_url() . 'student/delivery/update_text/'.$delivery_id.'/'.$homework_code);?>
			                        <div class="b-t padded-v-big">
			                            ...
			                            <div class="row">
			                                <div class="col-sm-12">
                                                <textarea class="full-width form-control" placeholder="<?php echo getEduAppGTLang('send_teacher_comment');?>" name="comment" rows="5"><?php echo $query->row()->student_comment;?></textarea>
		                        ...
		                        <div class="table-responsive">
		                            <table class="table table-lightbor table-lightfont">
			                            ...
			                            <tr>
				                            ...
				                            <?php endif;?>
				                            <?php if($query->num_rows() > 0):?>
				                                <a class="btn btn-rounded btn-sm btn-primary text-white"><?php $mark =$this->db->get_where('deliveries', array('homework_code' => $homework_code, 'student_id' => $this->session->userdata('login_user_id')))->row()->mark; if($mark > 0) echo $mark; else echo getEduAppGTLang('waiting_mark');?></a>
				                            <?php endif;?>
				                            </td>
			                            </tr>
			                            <tr>
