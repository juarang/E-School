                            		    ...
                            		    ?>
          							        <tr> 
										        ...
										        <td>
                                                    ...
                                                    <a class="btn btn-rounded btn-sm btn-danger text-white"><?php if($this->db->get_where('mark' , array('subject_id' => $row3['subject_id'], 'exam_id' => $row2['exam_id'], 'student_id' => $this->session->userdata('login_user_id'), 'year' => $running_year))->row()->labtotal == 0) echo '0'; else echo $mark;?></a>
                                                    <?php endif;?>
                                                    <?php if($mark >= $min):?>
                                                    <a class="btn btn-rounded btn-sm btn-info text-white"><?php echo $this->db->get_where('mark' , array('subject_id' => $row3['subject_id'], 'exam_id' => $row2['exam_id'], 'student_id' => $this->session->userdata('login_user_id'), 'year' => $running_year))->row()->labtotal;?></a>
                                                    <?php endif;?>
                                                </td>
                                                ...
                                                <td><a class="btn btn-rounded btn-sm btn-primary text-white" href="<?php echo base_url();?>student/subject_marks/<?php echo $data;?>/"><?php echo getEduAppGTLang('view_all');?></a></td>
          							        </tr>
